 ...
 * because the stock endpoint's field allowlist has no `orders` field to
 * request — see the header comment on the backend route for the full
 * reasoning. The order count returned here is always a real, computed
 * number; this module has no mock path and must never default a missing
 * count to zero.
 */
